...
export class MedicationsService {
  ...
  getMedicationAlarms(patientId: number): Observable<MedicationAlarm[]> {
    return this.apiService.get<MedicationAlarm[]>(
  ...
  createMedicationAlarm(
    alarm: Omit<MedicationAlarm, "id" | "createdAt">,
  ): Observable<MedicationAlarm> {
  ...
  updateMedicationAlarm(
    alarmId: number,
    alarm: Partial<MedicationAlarm>,
  ): Observable<MedicationAlarm> {
    return this.apiService.put<MedicationAlarm>(
      `${this.endpoint}/${alarmId}`,
      alarm,
    );
  }
